...
class Walker extends Lint.RuleWalker {
	...
	private _validatedDecorator(decorator: any) {
		...
		if (missing.length) {
			...
			props.filter((prop: any) => rules.required[prop.name] || rules.forbidden[prop.name]).forEach((prop: any) => {
				...
				if (requiredPattern && !requiredPattern.test(value)) {
					this.addFailureAtNode(node, `Invalid value for property. ` + `Expected value to match "${requiredPattern}".`);
				} else if (forbiddenPattern && forbiddenPattern.test(value)) {
					this.addFailureAtNode(node, `Property value not allowed. ` + `Value should not match "${forbiddenPattern}".`);
				}
			});
		}
	}
