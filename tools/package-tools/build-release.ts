...
export function composeRelease(buildPackage: BuildPackage) {
	...
	if (buildPackage.exportsSecondaryEntryPointsAtRoot) {
		...
		const es2015Exports = buildPackage.secondaryEntryPoints.map(p => `export * from './${p}';`).join('\n');
...
function createFilesForSecondaryEntryPoint(buildPackage: BuildPackage, releasePath: string) {
	...
	buildPackage.secondaryEntryPoints.forEach(entryPointName => {
		...
		copyFiles(join(packageOut, entryPointName), '**/*.+(d.ts|metadata.json)', join(entryPointDir, 'typings'));
		...
		createMetadataReexportFile(releasePath, `./${entryPointName}/index`, entryPointName, importAsName);
	});
}
