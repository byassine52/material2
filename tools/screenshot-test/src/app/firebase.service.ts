...
export class FirebaseService {
	...
	approvePullRequest() {
		return this._databaseRef()
			.child('approved')
			.child(this.screenshotResultSummary.sha)
			.set(Date.now());
	}
	...
	getGithubStatus() {
		let url = `https://api.github.com/repos/${config.repoSlug}/commits/` + `${this.screenshotResultSummary.sha}/status`;
		return this._httpClient
			.get<{ statuses: any[] }>(url)
			.toPromise()
			.then(response => {
				const screenshotStatus = response.statuses.find(status => status.context === 'Screenshot Tests');
