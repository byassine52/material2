...
import { buildConfig, copyFiles, buildScssTask, sequenceTask, watchFiles } from 'material2-build-tools';
import {
	cdkPackage,
...
task(':watch:devapp', () => {
	...
	watchFiles(join(momentAdapterPackage.sourceDir, '**/*'), ['material-moment-adapter:build-no-bundles']);
	watchFiles(join(materialExperimentalPackage.sourceDir, '**/*'), ['material-experimental:build-no-bundles']);
...
task(':build:devapp:material-with-styles', sequenceTask('material:build-no-bundles', ':build:devapp:scss'));
...
task('stage-deploy:devapp', ['build:devapp'], () => {
	...
	copyFiles(cdkExperimentalPackage.outputDir, '**/*.+(js|map)', join(outDir, 'dist/packages/cdk-experimental'));
	copyFiles(materialPackage.outputDir, '**/prebuilt/*.+(css|map)', join(outDir, 'dist/packages/material'));
});
