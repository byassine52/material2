...
task('stylelint', execNodeTask('stylelint', [stylesGlob, '--config', 'stylelint-config.json', '--syntax', 'scss']));
	...
	for (let paths = getChildPaths('.'); paths.length; ) {
		paths = Array.prototype.concat(
			...paths
				...
				.filter(
					path => !ignoredPaths.reduce((isIgnored, ignoredPath) => isIgnored || ignoredPath.match('/' + path), false)
				)
				// Remove paths that match an owned path.
				.filter(path => !ownedPaths.reduce((isOwned, ownedPath) => isOwned || isOwnedBy(path, ownedPath), false))
