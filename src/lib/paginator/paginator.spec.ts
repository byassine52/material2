...
describe('MatPaginator', () => {
	...
	it('should default the page size options to the page size if no options provided', () => {
		...
		expect(withoutOptionsAppFixture.componentInstance.paginator._displayedPageSizeOptions).toEqual([10]);
	});
	...
	it('should be able to change the page size while keeping the first item present', () => {
		...
		firstPageItemIndex = paginationEvent ? paginationEvent.pageIndex * paginationEvent.pageSize : null;
	...
	it('should be able to hide the page size select', () => {
		...
		expect(element.querySelector('.mat-paginator-page-size')).toBeTruthy('Expected select to be rendered.');

		fixture.componentInstance.hidePageSize = true;
		fixture.detectChanges();

		expect(element.querySelector('.mat-paginator-page-size')).toBeNull('Expected select to be removed.');
	});
});
