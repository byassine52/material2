...
describe('MatTabNavBar', () => {
	...
	describe('basic behavior', () => {
		...
		it('should add the disabled class if disabled', () => {
			...
			expect(tabLinkElements.every(tabLinkEl => !tabLinkEl.classList.contains('mat-tab-disabled'))).toBe(
				true,
		...
		it('should update the disableRipple property on each tab link', () => {
			const tabLinkElements = fixture.debugElement
				.queryAll(By.directive(MatTabLink))
				.map(tabLinkDebug => tabLinkDebug.componentInstance) as MatTabLink[];

			expect(tabLinkElements.every(tabLink => !tabLink.disableRipple)).toBe(
				true,
				'Expected every tab link to have ripples enabled'
			);
			...
			expect(tabLinkElements.every(tabLink => tabLink.disableRipple)).toBe(
				true,
				'Expected every tab link to have ripples disabled'
			);
		});
		...
		it('should show ripples for tab links', () => {
			...
			expect(tabLink.querySelectorAll('.mat-ripple-element').length).toBe(
				1,
		...
		it('should be able to disable ripples on a tab link', () => {
			...
			expect(tabLinkElement.querySelectorAll('.mat-ripple-element').length).toBe(
				0,
				'Expected no ripple to show up if ripples are disabled.'
			);
		});
		...
		it(
			'should re-align the ink bar when the window is resized',
			fakeAsync(() => {
				const inkBar = fixture.componentInstance.tabNavBar._inkBar;
	...
	it('should clean up the ripple event handlers on destroy', () => {
		...
		expect(link.querySelector('.mat-ripple-element')).toBeFalsy(
