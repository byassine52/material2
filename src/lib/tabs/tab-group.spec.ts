...
describe('MatTabGroup', () => {
	...
	describe('basic behavior', () => {
		...
		it(
			'should support two-way binding for selectedIndex',
			async(() => {
		...
		it(
			'should change tabs based on selectedIndex',
			fakeAsync(() => {
				let component = fixture.componentInstance;
		...
		it('should update tab positions when selected index is changed', () => {
			fixture.detectChanges();
			const component: MatTabGroup = fixture.debugElement.query(By.css('mat-tab-group')).componentInstance;
		...
		it('should show ripples for tab-group labels', () => {
			...
			expect(testElement.querySelectorAll('.mat-ripple-element').length).toBe(
				0,
		...
		it(
			'should fire animation done event',
			fakeAsync(() => {
				fixture.detectChanges();
	...
	describe('async tabs', () => {
		...
		it(
			'should show tabs when they are available',
			async(() => {
	...
	describe('with simple api', () => {
		...
		beforeEach(() => {
			...
			tabGroup = fixture.debugElement.query(By.directive(MatTabGroup)).componentInstance as MatTabGroup;
		});

		it(
			'should support a tab-group with the simple api',
			async(() => {
		...
		it(
			'should only have the active tab in the DOM',
			async(() => {
	...
	function checkSelectedIndex(expectedIndex: number, fixture: ComponentFixture<any>) {
		...
		let tabLabelElement = fixture.debugElement.query(By.css(`.mat-tab-label:nth-of-type(${expectedIndex + 1})`))
			.nativeElement;
		expect(tabLabelElement.classList.contains('mat-tab-label-active')).toBe(true);

		let tabContentElement = fixture.debugElement.query(By.css(`mat-tab-body:nth-of-type(${expectedIndex + 1})`))
			.nativeElement;
...
describe('nested MatTabGroup with enabled animations', () => {
	...
	it(
		'should not throw when creating a component with nested tab groups',
		async(() => {
			expect(() => {
