...
import { ChangeDetectionStrategy, Component, DebugElement, ElementRef, ViewChild, NgZone } from '@angular/core';
...
describe('MatTooltip', () => {
	...
	describe('basic usage', () => {
		...
		it(
			'should show and hide the tooltip',
			fakeAsync(() => {
		...
		it(
			'should be able to re-open a tooltip if it was closed by detaching the overlay',
			fakeAsync(() => {
				tooltipDirective.show();
				tick(0);
				expect(tooltipDirective._isTooltipVisible()).toBe(true);
		...
		it(
			'should be able to override the default show and hide delays',
			fakeAsync(() => {
		...
		it(
			'should set a css class on the overlay panel element',
			fakeAsync(() => {
				tooltipDirective.show();
				fixture.detectChanges();
				tick(0);
		...
		it(
			'should hide if disabled while visible',
			fakeAsync(() => {
		...
		it(
			'should hide if the message is cleared while the tooltip is open',
			fakeAsync(() => {
				tooltipDirective.show();
				fixture.detectChanges();
				tick(0);
		...
		it(
			'should not follow through with hide if show is called after',
			fakeAsync(() => {
				tooltipDirective.show();
				tick(0); // Tick for the show delay (default is 0)
				expect(tooltipDirective._isTooltipVisible()).toBe(true);
		...
		it(
			'should be able to modify the tooltip message',
			fakeAsync(() => {
				assertTooltipInstance(tooltipDirective, false);
		...
		it(
			'should allow extra classes to be set on the tooltip',
			fakeAsync(() => {
				assertTooltipInstance(tooltipDirective, false);
		...
		it(
			'should be removed after parent destroyed',
			fakeAsync(() => {
				tooltipDirective.show();
				tick(0); // Tick for the show delay (default is 0)
				expect(tooltipDirective._isTooltipVisible()).toBe(true);
		...
		it(
			'should not try to dispose the tooltip when destroyed and done hiding',
			fakeAsync(() => {
				tooltipDirective.show();
				fixture.detectChanges();
				tick(150);
		...
		it(
			'should pass the layout direction to the tooltip',
			fakeAsync(() => {
				dir.value = 'rtl';
		...
		it(
			'should be able to set the tooltip message as a number',
			fakeAsync(() => {
		...
		it(
			'should hide when clicking away',
			fakeAsync(() => {
				tooltipDirective.show();
				tick(0);
				fixture.detectChanges();
		...
		it(
			'should not hide immediately if a click fires while animating',
			fakeAsync(() => {
				tooltipDirective.show();
				tick(0);
				fixture.detectChanges();
		...
		it(
			'should not show the tooltip on progammatic focus',
			fakeAsync(() => {
				assertTooltipInstance(tooltipDirective, false);
	...
	describe('scrollable usage', () => {
		...
		it(
			'should hide tooltip if clipped after changing positions',
			fakeAsync(() => {
				assertTooltipInstance(tooltipDirective, false);
		...
		it(
			'should execute the `hide` call, after scrolling away, inside the NgZone',
			fakeAsync(() => {
	...
	describe('with OnPush', () => {
		...
		it(
			'should have rendered the tooltip text on init',
			fakeAsync(() => {
