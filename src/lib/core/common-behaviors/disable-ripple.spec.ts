...
describe('mixinDisableRipple', () => {
	it('should augment an existing class with a disableRipple property', () => {
		...
		expect(instance.disableRipple).toBe(false, 'Expected the mixed-into class to have a disable-ripple property');

		instance.disableRipple = true;

		expect(instance.disableRipple).toBe(
			true,
	...
	it('should coerce values being passed to the disableRipple property', () => {
		...
		expect(instance.disableRipple).toBe(false, 'Expected disableRipple to be set to false initially');
