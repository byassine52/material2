...
describe('MixinColor', () => {
	it('should augment an existing class with a color property', () => {
		...
		expect(instance.color).toBe('accent', 'Expected the mixed-into class to have an updated color property');
	});
	...
	it('should remove old color classes if new color is set', () => {
		...
		expect(instance.testElement.classList).toContain(
			'mat-primary',
			'Expected the element to have the "mat-primary" class set'
		);
		...
		expect(instance.testElement.classList).toContain(
			'mat-accent',
			'Expected the element to have the "mat-accent" class set'
		);
	});
	...
	it('should allow having no color set', () => {
		...
		expect(instance.testElement.classList.length).toBe(0, 'Expected the element to have no color class set.');
	});
	...
	it('should allow having a default color if specified', () => {
		...
		expect(instance.testElement.classList).toContain(
			'mat-accent',
			'Expected the element to have the "mat-accent" class by default.'
		);
		...
		expect(instance.testElement.classList).toContain(
			'mat-accent',
			'Expected the default color "mat-accent" to be set.'
		);
	});
});
