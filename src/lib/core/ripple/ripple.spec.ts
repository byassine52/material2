...
import { MatRipple, MatRippleModule, MAT_RIPPLE_GLOBAL_OPTIONS, RippleState, RippleGlobalOptions } from './index';
...
describe('MatRipple', () => {
	...
	describe('basic ripple', () => {
		...
		it(
			'should launch ripples on touchstart',
			fakeAsync(() => {
		...
		it(
			'should remove ripples after mouseup',
			fakeAsync(() => {
				dispatchMouseEvent(rippleTarget, 'mousedown');
		...
		it(
			'should not hide ripples while animating.',
			fakeAsync(() => {
				// Calculates the duration for fading-in and fading-out the ripple.
		...
		it('creates manual ripples with the default ripple config', () => {
			...
			expect(parseFloat(rippleElement.style.left as string)).toBeCloseTo(TARGET_WIDTH / 2 - radius, 1);
			expect(parseFloat(rippleElement.style.top as string)).toBeCloseTo(TARGET_HEIGHT / 2 - radius, 1);
		});
	...
	describe('manual ripples', () => {
		...
		it(
			'should allow persistent ripple elements',
			fakeAsync(() => {
				expect(rippleTarget.querySelectorAll('.mat-ripple-element').length).toBe(0);
		...
		it(
			'should remove ripples that are not done fading-in',
			fakeAsync(() => {
				expect(rippleTarget.querySelectorAll('.mat-ripple-element').length).toBe(0);
				...
				expect(rippleTarget.querySelectorAll('.mat-ripple-element').length).toBe(
					0,
		...
		it(
			'should properly set ripple states',
			fakeAsync(() => {
				expect(rippleTarget.querySelectorAll('.mat-ripple-element').length).toBe(0);
		...
		it(
			'should allow setting a specific animation config for a ripple',
			fakeAsync(() => {
				expect(rippleTarget.querySelectorAll('.mat-ripple-element').length).toBe(0);
	...
	describe('global ripple options', () => {
		...
		function createTestComponent(rippleConfig: RippleGlobalOptions, testComponent: any = BasicRippleContainer) {
		...
		it(
			'should support changing the baseSpeedFactor',
			fakeAsync(() => {
		...
		it(
			'should combine individual speed factor with baseSpeedFactor',
			fakeAsync(() => {
				createTestComponent({ baseSpeedFactor: 0.5 });
		...
		it(
			'should support changing the animation duration',
			fakeAsync(() => {
				createTestComponent({
	...
	describe('configuring behavior', () => {
		...
		it('allows specifying custom trigger element', () => {
			let alternateTrigger = fixture.debugElement.nativeElement.querySelector('.alternateTrigger') as HTMLElement;
		...
		it(
			'should be able to specify animation config through binding',
			fakeAsync(() => {
