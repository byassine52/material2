...
describe('MatOption component', () => {
	...
	describe('ripples', () => {
		...
		it('should show ripples by default', () => {
			...
			expect(optionNativeElement.querySelectorAll('.mat-ripple-element').length).toBe(
				0,
			...
			expect(optionNativeElement.querySelectorAll('.mat-ripple-element').length).toBe(
				1,
				'Expected one ripple to show up after a fake click.'
			);
		});
		...
		it('should not show ripples if the option is disabled', () => {
			...
			expect(optionNativeElement.querySelectorAll('.mat-ripple-element').length).toBe(
				0,
				'Expected no ripples to show up after click on a disabled option.'
			);
		});
	});
});
