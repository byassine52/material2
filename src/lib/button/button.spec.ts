...
describe('MatButton', () => {
	...
	describe('button[mat-fab]', () => {
		it('should have accent palette by default', () => {
			...
			expect(fabButtonDebugEl.nativeElement.classList).toContain(
				'mat-accent',
				'Expected fab buttons to use accent palette by default'
			);
		});
	});
	...
	describe('button[mat-mini-fab]', () => {
		it('should have accent palette by default', () => {
			...
			expect(miniFabButtonDebugEl.nativeElement.classList).toContain(
				'mat-accent',
				'Expected mini-fab buttons to use accent palette by default'
			);
		});
	});
	...
	describe('a[mat-button]', () => {
		...
		it('should not add aria-disabled attribute if disabled is false', () => {
			...
			expect(buttonDebugElement.nativeElement.getAttribute('disabled')).toBeNull('Expect disabled="false"');

			testComponent.isDisabled = false;
			fixture.detectChanges();
			expect(buttonDebugElement.nativeElement.getAttribute('aria-disabled')).toBe('false', 'Expect no aria-disabled');
			expect(buttonDebugElement.nativeElement.getAttribute('disabled')).toBeNull('Expect no disabled');
		});
	});
