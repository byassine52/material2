import { inject, ComponentFixture, TestBed, fakeAsync, tick, flush } from '@angular/core/testing';
...
describe('MatSnackBar', () => {
	...
	it('should have the role of alert', () => {
		...
		expect(containerElement.getAttribute('role')).toBe('alert', 'Expected snack bar container to have role="alert"');
	});

	it(
		'should open and close a snackbar without a ViewContainerRef',
		fakeAsync(() => {
			...
			expect(overlayContainerElement.childNodes.length).toBe(
				0,
				'Expected snack bar to be dismissed without a ViewContainerRef'
			);
		})
	);
	...
	it('should open a simple message with a button', () => {
		...
		expect(snackBarRef.instance instanceof SimpleSnackBar).toBe(
			true,
			'Expected the snack bar content component to be SimpleSnackBar'
		);
		expect(snackBarRef.instance.snackBarRef).toBe(
			snackBarRef,
		...
		expect(messageElement.textContent).toContain(
			simpleMessage,
			`Expected the snack bar message to be '${simpleMessage}'`
		);

		let buttonElement = overlayContainerElement.querySelector('button.mat-simple-snackbar-action')!;
		expect(buttonElement.tagName).toBe('BUTTON', 'Expected snack bar action label to be a <button>');
	...
	it('should open a simple message with no button', () => {
		...
		expect(overlayContainerElement.querySelector('button.mat-simple-snackbar-action')).toBeNull(
			'Expected the query selection for action label to be null'
		);
	});

	it(
		'should dismiss the snack bar and remove itself from the view',
		fakeAsync(() => {
			let config: MatSnackBarConfig = { viewContainerRef: testViewContainerRef };
	...
	it(
		'should be able to get dismissed through the service',
		fakeAsync(() => {
	...
	it(
		'should clean itself up when the view container gets destroyed',
		fakeAsync(() => {
			...
			expect(overlayContainerElement.childElementCount).toBe(
				0,
				'Expected snack bar to be removed after the view container was destroyed'
			);
		})
	);
	...
	it('should set the animation state to visible on entry', () => {
		...
		viewContainerFixture.detectChanges();
		expect(snackBarRef.containerInstance._animationState).toBe(
			'visible-bottom',
			`Expected the animation state would be 'visible-bottom'.`
		);
		snackBarRef.dismiss();
		...
		expect(snackBarRef.containerInstance._animationState).toBe(
			'hidden-bottom',
			`Expected the animation state would be 'hidden-bottom'.`
		);
	});
	...
	it(
		`should set the old snack bar animation state to complete and the new snack bar animation
      state to visible on entry of new snack bar`,
		fakeAsync(() => {
			...
			expect(snackBarRef2.containerInstance._animationState).toBe(
				'visible-bottom',
				`Expected the animation state would be 'visible-bottom'.`
			);
		})
	...
	it(
		'should open a new snackbar after dismissing a previous snackbar',
		fakeAsync(() => {
			let config: MatSnackBarConfig = { viewContainerRef: testViewContainerRef };
	...
	it(
		'should remove past snackbars when opening new snackbars',
		fakeAsync(() => {
	...
	it(
		'should remove snackbar if another is shown while its still animating open',
		fakeAsync(() => {
			snackBar.open('First snackbar');
			viewContainerFixture.detectChanges();
	...
	it(
		'should dismiss the snackbar when the action is called, notifying of both action and dismiss',
		fakeAsync(() => {
			...
			const actionButton = overlayContainerElement.querySelector('.mat-simple-snackbar-action') as HTMLButtonElement;
			actionButton.click();
			viewContainerFixture.detectChanges();
			tick();
	...
	it(
		'should allow manually dismissing with an action',
		fakeAsync(() => {
			const dismissCompleteSpy = jasmine.createSpy('dismiss complete spy');
			const actionCompleteSpy = jasmine.createSpy('action complete spy');
	...
	it(
		'should indicate in `afterClosed` whether it was dismissed by an action',
		fakeAsync(() => {
	...
	it(
		'should complete the onAction stream when not closing via an action',
		fakeAsync(() => {
			const actionCompleteSpy = jasmine.createSpy('action complete spy');
			const snackBarRef = snackBar.open('Some content');
			viewContainerFixture.detectChanges();
	...
	it(
		'should dismiss automatically after a specified timeout',
		fakeAsync(() => {
	...
	it(
		'should clear the dismiss timeout when dismissed before timeout expiration',
		fakeAsync(() => {
			let config = new MatSnackBarConfig();
	...
	describe('with custom component', () => {
		it('should open a custom component', () => {
			...
			expect(snackBarRef.instance instanceof BurritosNotification).toBe(
				true,
		...
		it('should inject the snack bar reference into the component', () => {
			...
			expect(snackBarRef.instance.snackBarRef).toBe(
				snackBarRef,
				'Expected component to have an injected snack bar reference.'
			);
		});
		...
		it('should be able to inject arbitrary user data', () => {
			...
			expect(snackBarRef.instance.data.burritoType).toBe(
				'Chimichanga',
...
describe('MatSnackBar with parent MatSnackBar', () => {
	...
	it(
		'should close snackBars opened by parent when opening from child',
		fakeAsync(() => {
			parentSnackBar.open('Pizza');
			fixture.detectChanges();
			tick(1000);

			expect(overlayContainerElement.textContent).toContain('Pizza', 'Expected a snackBar to be opened');
			...
			expect(overlayContainerElement.textContent).toContain(
				'Taco',
				'Expected parent snackbar msg to be dismissed by opening from child'
			);
		})
	);

	it(
		'should close snackBars opened by child when opening from parent',
		fakeAsync(() => {
...
describe('MatSnackBar Positioning', () => {
	...
	it(
		'should handle start based on direction (rtl)',
		fakeAsync(() => {
			snackBar.open(simpleMessage, simpleActionLabel, {
				verticalPosition: 'top',
	...
	it(
		'should handle start based on direction (ltr)',
		fakeAsync(() => {
			snackBar.open(simpleMessage, simpleActionLabel, {
				verticalPosition: 'top',
				horizontalPosition: 'start',
	...
	it(
		'should handle end based on direction (rtl)',
		fakeAsync(() => {
			snackBar.open(simpleMessage, simpleActionLabel, {
				verticalPosition: 'top',
	...
	it(
		'should handle end based on direction (ltr)',
		fakeAsync(() => {
			snackBar.open(simpleMessage, simpleActionLabel, {
				verticalPosition: 'top',
				horizontalPosition: 'end',
...
class BurritosNotification {
	constructor(public snackBarRef: MatSnackBarRef<BurritosNotification>, @Inject(MAT_SNACK_BAR_DATA) public data: any) {}
}
 ...
 */
const TEST_DIRECTIVES = [ComponentWithChildViewContainer, BurritosNotification, DirectiveWithViewContainer];
@NgModule({
