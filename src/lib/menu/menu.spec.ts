...
describe('MatMenu', () => {
	...
	it(
		'should close the menu when a click occurs outside the menu',
		fakeAsync(() => {
			const fixture = TestBed.createComponent(SimpleMenu);
			fixture.detectChanges();
			fixture.componentInstance.trigger.openMenu();
	...
	it(
		'should restore focus to the trigger when the menu was opened by keyboard',
		fakeAsync(() => {
			const fixture = TestBed.createComponent(SimpleMenu);
			fixture.detectChanges();
	...
	it(
		'should restore focus to the root trigger when the menu was opened by mouse',
		fakeAsync(() => {
			const fixture = TestBed.createComponent(SimpleMenu);
			fixture.detectChanges();
	...
	it(
		'should close the menu when pressing ESCAPE',
		fakeAsync(() => {
			const fixture = TestBed.createComponent(SimpleMenu);
			fixture.detectChanges();
			fixture.componentInstance.trigger.openMenu();
	...
	it(
		'should set the proper focus origin when opening by mouse',
		fakeAsync(() => {
			const fixture = TestBed.createComponent(SimpleMenu);
			fixture.detectChanges();
	...
	it(
		'should close the menu when using the CloseScrollStrategy',
		fakeAsync(() => {
	...
	describe('lazy rendering', () => {
		it(
			'should be able to render the menu content lazily',
			fakeAsync(() => {
		...
		it(
			'should focus the first menu item when opening a lazy menu via keyboard',
			fakeAsync(() => {
				let zone: MockNgZone;
		...
		it(
			'should be able to open the same menu with a different context',
			fakeAsync(() => {
	...
	describe('nested menu', () => {
		...
		it(
			'should toggle a nested menu when its trigger is hovered',
			fakeAsync(() => {
				compileTestComponent();
				instance.rootTriggerEl.nativeElement.click();
				fixture.detectChanges();
		...
		it(
			...
			fakeAsync(() => {
				...
				expect(overlay.querySelectorAll('.mat-menu-panel').length).toBe(3, 'Expected three open menus');
		...
		it('should open a nested menu when its trigger is clicked', () => {
			...
			levelOneTrigger.click();
			fixture.detectChanges();
			expect(overlay.querySelectorAll('.mat-menu-panel').length).toBe(
				2,
		...
		it(
			'should open and close a nested menu with arrow keys in ltr',
			fakeAsync(() => {
				compileTestComponent();
				instance.rootTriggerEl.nativeElement.click();
				fixture.detectChanges();
		...
		it(
			'should open and close a nested menu with the arrow keys in rtl',
			fakeAsync(() => {
				dir = 'rtl';
				fixture.destroy();
				compileTestComponent();
		...
		it('should not do anything with the arrow keys for a top-level menu', () => {
			...
			fixture.detectChanges();
			expect(overlay.querySelectorAll('.mat-menu-panel').length).toBe(1, 'Expected one menu to remain open');
		...
		it(
			'should close all of the menus when the backdrop is clicked',
			fakeAsync(() => {
				compileTestComponent();
				instance.rootTriggerEl.nativeElement.click();
				fixture.detectChanges();
				...
				expect(overlay.querySelectorAll('.mat-menu-panel').length).toBe(3, 'Expected three open menus');
				expect(overlay.querySelectorAll('.cdk-overlay-backdrop').length).toBe(1, 'Expected one backdrop element');
				expect(overlay.querySelectorAll('.mat-menu-panel, .cdk-overlay-backdrop')[0].classList).toContain(
		...
		it('should shift focus between the sub-menus', () => {
			...
			expect(overlay.querySelector('.mat-menu-panel')!.contains(document.activeElement)).toBe(
				true,
			...
			expect(overlay.querySelectorAll('.mat-menu-panel')[1].contains(document.activeElement)).toBe(
				true,
			...
			expect(overlay.querySelectorAll('.mat-menu-panel')[2].contains(document.activeElement)).toBe(
				true,
		...
		it(
			'should fall back to aligning to the right edge of the trigger in rtl',
			fakeAsync(() => {
				dir = 'rtl';
				compileTestComponent();
				instance.rootTriggerEl.nativeElement.style.position = 'fixed';
		...
		it(
			'should close all of the menus when an item is clicked',
			fakeAsync(() => {
				compileTestComponent();
				instance.rootTriggerEl.nativeElement.click();
				fixture.detectChanges();
		...
		it('should increase the sub-menu elevation based on its depth', () => {
			...
			expect(menus[0].classList).toContain('mat-elevation-z2', 'Expected root menu to have base elevation.');
			expect(menus[1].classList).toContain('mat-elevation-z3', 'Expected first sub-menu to have base elevation + 1.');
			expect(menus[2].classList).toContain('mat-elevation-z4', 'Expected second sub-menu to have base elevation + 2.');
		});
		...
		it(
			...
			fakeAsync(() => {
				...
				expect(lastMenu.classList).toContain('mat-elevation-z4', 'Expected menu to have the base elevation plus two.');
				...
				expect(lastMenu.classList).not.toContain('mat-elevation-z4', 'Expected menu not to maintain old elevation.');
				expect(lastMenu.classList).toContain('mat-elevation-z2', 'Expected menu to have the proper updated elevation.');
			})
		);
		...
		it('should not increase the elevation if the user specified a custom one', () => {
			...
			expect(menuClasses).toContain('mat-elevation-z24', 'Expected user elevation to be maintained');
			expect(menuClasses).not.toContain('mat-elevation-z3', 'Expected no stacked elevation.');
		});

		it(
			'should close all of the menus when the root is closed programmatically',
			fakeAsync(() => {
				compileTestComponent();
				instance.rootTrigger.openMenu();
				fixture.detectChanges();
		...
		it(
			'should toggle a nested menu when its trigger is added after init',
			fakeAsync(() => {
				compileTestComponent();
				instance.rootTriggerEl.nativeElement.click();
				fixture.detectChanges();
				...
				expect(lazyTrigger.classList).toContain('mat-menu-item-highlighted', 'Expected the trigger to be highlighted');
				expect(overlay.querySelectorAll('.mat-menu-panel').length).toBe(2, 'Expected two open menus');
			})
		);
		...
		it(
			'should handle the items being rendered in a repeater',
			fakeAsync(() => {
		...
		it(
			'should not re-focus a child menu trigger when hovering another trigger',
			fakeAsync(() => {
				compileTestComponent();
