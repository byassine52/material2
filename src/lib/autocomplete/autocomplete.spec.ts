...
describe('MatAutocomplete', () => {
	...
	afterEach(
		inject([OverlayContainer], (currentOverlayContainer: OverlayContainer) => {
	...
	describe('panel toggling', () => {
		...
		it(
			'should not open the panel on focus if the input is readonly',
			fakeAsync(() => {
		...
		it(
			'should not open using the arrow keys when the input is readonly',
			fakeAsync(() => {
				const trigger = fixture.componentInstance.trigger;
				input.readOnly = true;
				fixture.detectChanges();
		...
		it(
			'should close the panel when the user clicks away',
			fakeAsync(() => {
				dispatchFakeEvent(input, 'focusin');
				fixture.detectChanges();
		...
		it(
			'should close the panel when the user taps away on a touch device',
			fakeAsync(() => {
		...
		it(
			'should close the panel when an option is clicked',
			fakeAsync(() => {
				dispatchFakeEvent(input, 'focusin');
				fixture.detectChanges();
				zone.simulateZoneExit();
				...
				expect(overlayContainerElement.textContent).toEqual('', `Expected clicking an option to close the panel.`);
			})
		);

		it(
			'should close the panel when a newly created option is clicked',
			fakeAsync(() => {
				dispatchFakeEvent(input, 'focusin');
				fixture.detectChanges();
				zone.simulateZoneExit();
				...
				expect(overlayContainerElement.textContent).toEqual('', `Expected clicking a new option to close the panel.`);
			})
		);
		...
		it('should close the panel programmatically', () => {
			...
			expect(overlayContainerElement.textContent).toEqual('', `Expected closing programmatically to close the panel.`);
		});
		...
		it(
			'should hide the panel when the options list is empty',
			fakeAsync(() => {
				dispatchFakeEvent(input, 'focusin');
				fixture.detectChanges();
				...
				expect(panel.classList).toContain('mat-autocomplete-visible', `Expected panel to start out visible.`);
				...
				expect(panel.classList).toContain('mat-autocomplete-hidden', `Expected panel to hide itself when empty.`);
			})
		);

		it(
			'should keep the label floating until the panel closes',
			fakeAsync(() => {
				fixture.componentInstance.trigger.openPanel();
				expect(fixture.componentInstance.formField.floatLabel).toEqual(
					'always',
					'Expected label to float as soon as panel opens.'
				);
		...
		it('should not open the panel when the `input` event is invoked on a non-focused input', () => {
			...
			expect(fixture.componentInstance.trigger.panelOpen).toBe(false, `Expected panel state to stay closed.`);
		});

		it(
			'should not mess with label placement if set to never',
			fakeAsync(() => {
				fixture.componentInstance.floatLabel = 'never';
				fixture.detectChanges();

				fixture.componentInstance.trigger.openPanel();
				expect(fixture.componentInstance.formField.floatLabel).toEqual('never', 'Expected label to stay static.');
				flush();
				fixture.detectChanges();
				...
				expect(fixture.componentInstance.formField.floatLabel).toEqual(
					'never',
					'Expected label to stay in static state after close.'
				);
			})
		);

		it(
			'should not mess with label placement if set to always',
			fakeAsync(() => {
				...
				expect(fixture.componentInstance.formField.floatLabel).toEqual(
					'always',
					'Expected label to stay elevated on open.'
				);
				flush();
				fixture.detectChanges();
				...
				expect(fixture.componentInstance.formField.floatLabel).toEqual(
					'always',
					'Expected label to stay elevated after close.'
				);
			})
		);

		it(
			'should toggle the visibility when typing and closing the panel',
			fakeAsync(() => {
				fixture.componentInstance.trigger.openPanel();
				tick();
				fixture.detectChanges();

				expect(overlayContainerElement.querySelector('.mat-autocomplete-panel')!.classList).toContain(
	...
	describe('forms integration', () => {
		...
		it(
			...
			fakeAsync(() => {
				...
				expect(input.value).toContain('California', `Expected text field to fill with selected value.`);
			})
		);
		...
		it(
			...
			fakeAsync(() => {
				...
				fixture.detectChanges();
				expect(input.value).toContain('test value', `Expected input to fall back to selected option's value.`);
			})
		);
		...
		it(
			...
			fakeAsync(() => {
				...
				expect(input.value).toContain('Alabama', `Expected input to fill with matching option's viewValue.`);
			})
		);
		...
		it('should disable input in view when disabled programmatically', () => {
			...
			expect(formFieldElement.classList.contains('mat-form-field-disabled')).toBe(
				false,
			...
			expect(input.disabled).toBe(true, `Expected input to be disabled in view when disabled programmatically.`);
			expect(formFieldElement.classList.contains('mat-form-field-disabled')).toBe(
				true,
		...
		it('should mark the autocomplete control as touched on blur', () => {
			fixture.componentInstance.trigger.openPanel();
			fixture.detectChanges();
			expect(fixture.componentInstance.stateCtrl.touched).toBe(false, `Expected control to start out untouched.`);

			dispatchFakeEvent(input, 'blur');
			fixture.detectChanges();

			expect(fixture.componentInstance.stateCtrl.touched).toBe(true, `Expected control to become touched on blur.`);
		});
	...
	describe('keyboard events', () => {
		...
		it('should set the active item to the first option when DOWN key is pressed', () => {
			...
			const optionEls = overlayContainerElement.querySelectorAll('mat-option') as NodeListOf<HTMLElement>;
			...
			expect(componentInstance.trigger.activeOption === componentInstance.options.toArray()[1]).toBe(
				true,
		...
		it('should set the active item to the last option when UP key is pressed', () => {
			...
			expect(componentInstance.trigger.panelOpen).toBe(true, 'Expected first up press to open the pane.');
		...
		it(
			...
			fakeAsync(() => {
				...
				fixture.detectChanges();
				expect(input.value).toContain('Alabama', `Expected text field to fill with selected value on ENTER.`);
			})
		);
		...
		it(
			...
			fakeAsync(() => {
				...
				expect(ENTER_EVENT.defaultPrevented).toBe(true, 'Expected the default action to have been prevented.');
			})
		);
		...
		it(
			...
			fakeAsync(() => {
				...
				expect(overlayContainerElement.textContent).toEqual('', `Expected panel to close after ENTER key.`);
		...
		it('should scroll to active options that are above the panel', () => {
			...
			expect(scrollContainer.scrollTop).toEqual(48, `Expected panel to scroll up when option is above panel.`);
		});
		...
		it(
			...
			fakeAsync(() => {
				...
				expect(overlayContainerElement.querySelector('.mat-autocomplete-panel')).toBeTruthy(
				...
				expect(overlayContainerElement.querySelector('.mat-autocomplete-panel')).toBeFalsy(
	...
	describe('option groups', () => {
		...
		it(
			...
			fakeAsync(() => {
				...
				// 288 - 256 + 96 = 128
				expect(container.scrollTop).toBe(128, 'Expected panel to reveal the sixth option.');
			})
		);
		...
		it(
			...
			fakeAsync(() => {
				...
				expect(container.scrollTop).toBe(96, 'Expected panel to scroll up when option is above panel.');
			})
		);
	});
	...
	describe('aria', () => {
		...
		it('should set role of input to combobox', () => {
			expect(input.getAttribute('role')).toEqual('combobox', 'Expected role of input to be combobox.');
		});
		...
		it('should set role of autocomplete panel to listbox', () => {
			...
			expect(panel.getAttribute('role')).toEqual('listbox', 'Expected role of the panel to be listbox.');
		});
		...
		it('should set aria-owns based on the attached autocomplete', () => {
			...
			expect(input.getAttribute('aria-owns')).toEqual(
	...
	describe('Fallback positions', () => {
		...
		it(
			...
			fakeAsync(() => {
				...
				expect(Math.floor(inputTop)).toEqual(Math.floor(panelBottom), `Expected panel to fall back to above position.`);
			})
		);
	...
	describe('misc', () => {
		...
		it('should work when input is wrapped in ngIf', () => {
			...
			expect(overlayContainerElement.textContent).toContain('One', `Expected panel to display when input is focused.`);
			expect(overlayContainerElement.textContent).toContain('Two', `Expected panel to display when input is focused.`);
		});
