...
describe('MatCheckbox', () => {
	...
	describe('basic behaviors', () => {
		...
		it(
			'should set indeterminate to false when input clicked',
			fakeAsync(() => {
				testComponent.isIndeterminate = true;
				fixture.detectChanges();
				...
				expect(inputElement.checked).toBe(true);
				expect(testComponent.isIndeterminate).toBe(true);
				expect(inputElement.getAttribute('aria-checked')).toBe('true', 'Expect aria checked to be true');
		...
		it(
			'should change from indeterminate to checked on click',
			fakeAsync(() => {
				testComponent.isChecked = false;
				testComponent.isIndeterminate = true;
				fixture.detectChanges();
		...
		it(
			'should overwrite indeterminate state when clicked',
			fakeAsync(() => {
				testComponent.isIndeterminate = true;
				fixture.detectChanges();
		...
		it(
			'should trigger a change event when the native input does',
			fakeAsync(() => {
		...
		it(
			'should show a ripple when focused by a keyboard action',
			fakeAsync(() => {
				expect(fixture.nativeElement.querySelectorAll('.mat-ripple-element').length).toBe(
					0,
					'Expected no ripples on load.'
				);
				...
				expect(fixture.nativeElement.querySelectorAll('.mat-ripple-element').length).toBe(
					1,
					'Expected ripple after element is focused.'
				);
				...
				expect(fixture.nativeElement.querySelectorAll('.mat-ripple-element').length).toBe(
					0,
					'Expected no ripple after element is blurred.'
				);
			})
		);
		...
		describe('state transition css classes', () => {
			...
			it('should transition unchecked -> indeterminate -> unchecked', () => {
				...
				expect(checkboxNativeElement.classList).toContain('mat-checkbox-anim-unchecked-indeterminate');

				testComponent.isIndeterminate = false;
				fixture.detectChanges();

				expect(checkboxNativeElement.classList).not.toContain('mat-checkbox-anim-unchecked-indeterminate');
				expect(checkboxNativeElement.classList).toContain('mat-checkbox-anim-indeterminate-unchecked');
			});
		...
		describe(`when MAT_CHECKBOX_CLICK_ACTION is 'check'`, () => {
			...
			it(
				'should not set `indeterminate` to false on click if check is set',
				fakeAsync(() => {
					testComponent.isIndeterminate = true;
					inputElement.click();
		...
		describe(`when MAT_CHECKBOX_CLICK_ACTION is 'noop'`, () => {
			...
			it(
				'should not change `indeterminate` on click if noop is set',
				fakeAsync(() => {
					testComponent.isIndeterminate = true;
					inputElement.click();
			...
			it(
				`should not change 'checked' or 'indeterminate' on click if noop is set`,
				fakeAsync(() => {
					testComponent.isChecked = true;
					testComponent.isIndeterminate = true;
					inputElement.click();
	...
	describe('with change event and no initial value', () => {
		...
		it(
			'should not emit a DOM event to the change output',
			fakeAsync(() => {
				fixture.detectChanges();
	...
	describe('with native tabindex attribute', () => {
		it(
			'should properly detect native tabindex attribute',
			fakeAsync(() => {
				...
				expect(checkbox.tabIndex).toBe(5, 'Expected tabIndex property to have been set based on the native attribute');
			})
		);
	});
	...
	describe('with ngModel', () => {
		...
		it(
			'should be in pristine, untouched, and valid states initially',
			fakeAsync(() => {
				flush();
	...
	describe('without label', () => {
		...
		beforeEach(() => {
			...
			checkboxInnerContainer = checkboxDebugEl.query(By.css('.mat-checkbox-inner-container')).nativeElement;
		});

		it('should remove margin for checkbox without a label', () => {
			fixture.detectChanges();

			expect(checkboxInnerContainer.classList).toContain('mat-checkbox-inner-container-no-side-margin');
		});
		...
		it('should not remove margin if initial label is set through binding', () => {
			...
			expect(checkboxInnerContainer.classList).not.toContain('mat-checkbox-inner-container-no-side-margin');
		});
		...
		it('should re-add margin if label is added asynchronously', () => {
			...
			fixture = TestBed.createComponent(CheckboxWithoutLabel);
			checkboxInnerContainer = fixture.debugElement.query(By.css('.mat-checkbox-inner-container')).nativeElement;
