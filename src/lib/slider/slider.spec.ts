...
describe('MatSlider without forms', () => {
	...
	describe('slider with set min and max', () => {
		...
		beforeEach(() => {
			...
			sliderWrapperElement = <HTMLElement>sliderNativeElement.querySelector('.mat-slider-wrapper');
			trackFillElement = <HTMLElement>sliderNativeElement.querySelector('.mat-slider-track-fill');
			ticksContainerElement = <HTMLElement>sliderNativeElement.querySelector('.mat-slider-ticks-container');
	...
	describe('slider with set tick interval', () => {
		...
		it('should be able to reset the tick interval after it has been set', () => {
			expect(sliderNativeElement.classList).toContain(
				'mat-slider-has-ticks',
				'Expected element to have ticks initially.'
			);
	...
	describe('slider with change handler', () => {
		...
		it(
			'should dispatch events when changing back to previously emitted value after ' + 'programmatically setting value',
			() => {
				expect(testComponent.onChange).not.toHaveBeenCalled();
	...
	describe('slider with direction and invert', () => {
		...
		it('should hide last tick when inverted and at min value', () => {
			...
			expect(sliderNativeElement.classList.contains('mat-slider-hide-last-tick')).toBe(
				true,
				'last tick should be hidden'
			);
		});
	});
	...
	describe('tabindex', () => {
		...
		it('should detect the native tabindex attribute', () => {
			...
			expect(slider.tabIndex).toBe(5, 'Expected the tabIndex to be set to the value of the native attribute.');
		});
	});
});
...
describe('MatSlider with forms module', () => {
	...
	beforeEach(
		async(() => {
			TestBed.configureTestingModule({
				imports: [MatSliderModule, ReactiveFormsModule, FormsModule, BidiModule],
				declarations: [SliderWithFormControl, SliderWithNgModel],
				providers: [
					{
						provide: HAMMER_GESTURE_CONFIG,
						useFactory: () => {
...
function dispatchSlideEventSequence(
	...
	startPercent: number,
	endPercent: number,
	gestureConfig: TestGestureConfig
): void {
	dispatchMouseenterEvent(sliderElement);
 ...
 * @param gestureConfig The gesture config for the test to handle emitting the slide events.
 */
function dispatchSlideEvent(sliderElement: HTMLElement, percent: number, gestureConfig: TestGestureConfig): void {
	let trackElement = sliderElement.querySelector('.mat-slider-wrapper')!;
	let dimensions = trackElement.getBoundingClientRect();
 ...
 * @param gestureConfig The gesture config for the test to handle emitting the slide events.
 */
function dispatchSlideStartEvent(sliderElement: HTMLElement, percent: number, gestureConfig: TestGestureConfig): void {
	let trackElement = sliderElement.querySelector('.mat-slider-wrapper')!;
	let dimensions = trackElement.getBoundingClientRect();
	let x = dimensions.left + dimensions.width * percent;
	let y = dimensions.top + dimensions.height * percent;
 ...
 * @param gestureConfig The gesture config for the test to handle emitting the slide events.
 */
function dispatchSlideEndEvent(sliderElement: HTMLElement, percent: number, gestureConfig: TestGestureConfig): void {
	let trackElement = sliderElement.querySelector('.mat-slider-wrapper')!;
	let dimensions = trackElement.getBoundingClientRect();
	let x = dimensions.left + dimensions.width * percent;
	let y = dimensions.top + dimensions.height * percent;
