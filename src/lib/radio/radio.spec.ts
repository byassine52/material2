...
describe('MatRadio', () => {
	...
	describe('inside of a group', () => {
		...
		it('should emit a change event from radio buttons', () => {
			...
			const spies = radioInstances.map((radio, index) => jasmine.createSpy(`onChangeSpy ${index} for ${radio.name}`));
		...
		it(
			'should show a ripple when focusing via the keyboard',
			fakeAsync(() => {
				expect(radioNativeElements[0].querySelectorAll('.mat-ripple-element').length).toBe(
					0,
					'Expected no ripples on init.'
				);
				...
				expect(radioNativeElements[0].querySelectorAll('.mat-ripple-element').length).toBe(
					1,
					'Expected one ripple after keyboard focus.'
				);
		...
		it('should not show ripples on disabled radio buttons', () => {
			...
			expect(radioNativeElements[0].querySelectorAll('.mat-ripple-element').length).toBe(
				0,
				'Expected a disabled radio button to not show ripples'
			);
			...
			expect(radioNativeElements[0].querySelectorAll('.mat-ripple-element').length).toBe(
				1,
				'Expected an enabled radio button to show ripples'
			);
		});
		...
		it('should apply class based on color attribute', () => {
			expect(radioNativeElements.every(radioEl => radioEl.classList.contains('mat-accent'))).toBe(
				true,
				'Expected every radio element to use the accent color by default.'
			);

			testComponent.color = 'primary';
			fixture.detectChanges();

			expect(radioNativeElements.every(radioEl => radioEl.classList.contains('mat-primary'))).toBe(
				true,
				'Expected every radio element to use the primary color from the binding.'
			);
	...
	describe('group with ngModel', () => {
		...
		it(
			'should write to the radio button based on ngModel',
			fakeAsync(() => {
	...
	describe('with tabindex', () => {
		...
		it('should allow specifying an explicit tabindex for a single radio-button', () => {
			...
			expect(radioButtonInput.tabIndex).toBe(0, 'Expected the tabindex to be set to "0" by default.');
