...
describe('MatTextareaAutosize', () => {
	...
	beforeEach(
		async(() => {
			TestBed.configureTestingModule({
				imports: [FormsModule, MatInputModule, MatStepperModule, MatTabsModule, NoopAnimationsModule],
				declarations: [
	...
	beforeEach(() => {
		...
		autosize = fixture.debugElement
			.query(By.directive(MatTextareaAutosize))
			.injector.get<MatTextareaAutosize>(MatTextareaAutosize);
	});
	...
	it('should set a min-width based on minRows', () => {
		...
		expect(parseInt(textarea.style.minHeight as string)).toBeGreaterThan(
			previousMinHeight,
	...
	it('should set a max-width based on maxRows', () => {
		...
		expect(parseInt(textarea.style.maxHeight as string)).toBeGreaterThan(
			previousMaxHeight,
	...
	it('should initially set the rows of a textarea to one', () => {
		expect(textarea.rows).toBe(1, 'Expected the directive to initially set the rows property to one.');

		fixture.componentInstance.minRows = 1;
		fixture.detectChanges();

		expect(textarea.rows).toBe(1, 'Expected the textarea to have the rows property set to one.');
		...
		expect(textarea.rows).toBe(
			1,
			'Expected the rows property to be set to one. ' + 'The amount of rows will be specified using CSS.'
		);

		expect(parseInt(textarea.style.minHeight as string)).toBeGreaterThan(
			previousMinHeight,
			'Expected the textarea to grow to two rows.'
		);
	});
	...
	it('should calculate the proper height based on the specified amount of max rows', () => {
		...
		expect(textarea.clientHeight).toBe(textarea.scrollHeight, 'Expected textarea to not have a vertical scrollbar.');

		fixture.componentInstance.maxRows = 5;
		fixture.detectChanges();

		expect(textarea.clientHeight).toBeLessThan(
			textarea.scrollHeight,
			'Expected textarea to have a vertical scrollbar.'
		);
	});
	...
	it('should not calculate wrong content height due to long placeholders', () => {
		...
		autosize = fixtureWithPlaceholder.debugElement
			.query(By.directive(MatTextareaAutosize))
			.injector.get<MatTextareaAutosize>(MatTextareaAutosize);
	...
	it(
		'should resize when an associated form control value changes',
		fakeAsync(() => {
    ...
    Some late visitor entreating entrance at my chamber door;—
			...
			expect(textarea.clientHeight).toBeGreaterThan(
				previousHeight,
				'Expected increased height when ngModel is updated.'
			);
		})
	);

	it(
		'should resize when the textarea value is changed programmatically',
		fakeAsync(() => {
			const previousHeight = textarea.clientHeight;
    ...
    `;
			...
			expect(textarea.clientHeight).toBeGreaterThan(previousHeight, 'Expected the textarea height to have increased.');
		})
	);
	...
	it(
		'should trigger a resize when the window is resized',
		fakeAsync(() => {
