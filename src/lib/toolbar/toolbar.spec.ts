...
describe('MatToolbar', () => {
	...
	describe('with single row', () => {
		...
		it('should not wrap the first row contents inside of a generated element', () => {
			expect(toolbarElement.firstElementChild!.tagName).toBe(
				'SPAN',
	...
	describe('with multiple rows', () => {
		it('should project each toolbar-row element inside of the toolbar', () => {
			...
			expect(fixture.debugElement.queryAll(By.css('.mat-toolbar > .mat-toolbar-row')).length).toBe(
				2,
				'Expected one toolbar row to be present while no content is projected.'
			);
		});
