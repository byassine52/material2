 ...
 */
import { animate, state, style, transition, trigger, keyframes, AnimationTriggerMetadata } from '@angular/animations';
	...
	indicatorToggle: trigger('indicatorToggle', [
		transition(
			...
			animate(
				...
				keyframes([style({ transform: 'translateY(25%)', opacity: 0 }), style({ transform: 'none', opacity: 1 })])
			)
		),
		transition(
			'asc => void',
