...
describe('Key managers', () => {
	...
	describe('ListKeyManager', () => {
		...
		beforeEach(() => {
			itemList.items = [new FakeFocusable('one'), new FakeFocusable('two'), new FakeFocusable('three')];
		...
		describe('Key events', () => {
			...
			function runDirectionalKeyTests() {
				it('should set subsequent items as active when the next key is pressed', () => {
					...
					expect(keyManager.activeItemIndex).toBe(1, 'Expected active item to be 1 after one next key event.');
					...
					keyManager.onKeydown(this.nextKeyEvent);
					expect(keyManager.activeItemIndex).toBe(2, 'Expected active item to be 2 after two next key events.');
				...
				it('should set first item active when the next key is pressed if no active item', () => {
					...
					expect(keyManager.activeItemIndex).toBe(
						0,
				...
				it('should skip disabled items', () => {
					...
					keyManager.onKeydown(this.nextKeyEvent);
					expect(keyManager.activeItemIndex).toBe(2, 'Expected active item to skip past disabled item on next event.');
					expect(keyManager.setActiveItem).not.toHaveBeenCalledWith(0);
					expect(keyManager.setActiveItem).not.toHaveBeenCalledWith(1);
					expect(keyManager.setActiveItem).toHaveBeenCalledWith(2);
					...
					keyManager.onKeydown(this.prevKeyEvent);
					expect(keyManager.activeItemIndex).toBe(0, 'Expected active item to skip past disabled item on up arrow.');
					expect(keyManager.setActiveItem).toHaveBeenCalledWith(0);
					expect(keyManager.setActiveItem).not.toHaveBeenCalledWith(1);
				});
				...
				it('should not move active item past either end of the list', () => {
					keyManager.onKeydown(this.nextKeyEvent);
					keyManager.onKeydown(this.nextKeyEvent);
					expect(keyManager.activeItemIndex).toBe(2, `Expected last item of the list to be active.`);

					// This next event would move active item past the end of the list
					keyManager.onKeydown(this.nextKeyEvent);
					expect(keyManager.activeItemIndex).toBe(2, `Expected active item to remain at the end of the list.`);

					keyManager.onKeydown(this.prevKeyEvent);
					keyManager.onKeydown(this.prevKeyEvent);
					expect(keyManager.activeItemIndex).toBe(0, `Expected first item of the list to be active.`);

					// This prev event would move active item past the beginning of the list
					keyManager.onKeydown(this.prevKeyEvent);
					expect(keyManager.activeItemIndex).toBe(0, `Expected active item to remain at the beginning of the list.`);
				});

				it('should not move active item to end when the last item is disabled', () => {
					itemList.items[2].disabled = true;
					keyManager.onKeydown(this.nextKeyEvent);
					expect(keyManager.activeItemIndex).toBe(1, `Expected second item of the list to be active.`);

					// This next key event would set active item to the last item, which is disabled
					keyManager.onKeydown(this.nextKeyEvent);
					expect(keyManager.activeItemIndex).toBe(1, `Expected the second item to remain active.`);
					expect(keyManager.setActiveItem).not.toHaveBeenCalledWith(2);
				});
		...
		describe('programmatic focus', () => {
			...
			it('should expose the active item correctly', () => {
				...
				expect(keyManager.activeItem).toBe(itemList.items[1], 'Expected the active item to match the second option.');

				keyManager.onKeydown(fakeKeyEvents.downArrow);
				expect(keyManager.activeItemIndex).toBe(2, 'Expected active item to be the third option.');
				expect(keyManager.activeItem).toBe(itemList.items[2], 'Expected the active item ID to match the third option.');
			});
			...
			it('should set the active item to the second item if the first one is disabled', () => {
				...
				keyManager.setFirstItemActive();
				expect(keyManager.activeItemIndex).toBe(1, `Expected the second item to be active if the first was disabled.`);
			});
			...
			it('should set the active item to the next enabled item if next is disabled', () => {
				...
				keyManager.setNextItemActive();
				expect(keyManager.activeItemIndex).toBe(2, `Expected setNextItemActive() to only set enabled items as active.`);
			});
			...
			it('should skip disabled items when setPreviousItemActive() is called', () => {
				...
				keyManager.onKeydown(fakeKeyEvents.downArrow);
				keyManager.onKeydown(fakeKeyEvents.downArrow);
				expect(keyManager.activeItemIndex).toBe(2, `Expected third item of the list to be active.`);

				keyManager.setPreviousItemActive();
				expect(keyManager.activeItemIndex).toBe(0, `Expected setPreviousItemActive() to skip the disabled item.`);
			});
		...
		describe('wrap mode', () => {
			it('should return itself to allow chaining', () => {
				expect(keyManager.withWrap()).toEqual(
					keyManager,
			...
			it('should set last item active when up arrow is pressed if no active item', () => {
				...
				expect(keyManager.activeItemIndex).toBe(2, 'Expected last item to be active on up arrow if no active item.');
				expect(keyManager.setActiveItem).not.toHaveBeenCalledWith(0);
				expect(keyManager.setActiveItem).toHaveBeenCalledWith(2);

				keyManager.onKeydown(fakeKeyEvents.downArrow);
				expect(keyManager.activeItemIndex).toBe(0, 'Expected active item to be 0 after wrapping back to beginning.');
				expect(keyManager.setActiveItem).toHaveBeenCalledWith(0);
			});
		});
		...
		describe('typeahead mode', () => {
			...
			it(
				'should debounce the input key presses',
				fakeAsync(() => {
			...
			it(
				'should focus the first item that starts with a letter',
				fakeAsync(() => {
			...
			it(
				'should focus the first item that starts with sequence of letters',
				fakeAsync(() => {
					keyManager.onKeydown(createKeyboardEvent('keydown', 84, undefined, 't')); // types "t"
			...
			it(
				'should cancel any pending timers if a navigation key is pressed',
				fakeAsync(() => {
					keyManager.onKeydown(createKeyboardEvent('keydown', 84, undefined, 't')); // types "t"
					keyManager.onKeydown(createKeyboardEvent('keydown', 72, undefined, 'h')); // types "h"
					keyManager.onKeydown(fakeKeyEvents.downArrow);
			...
			it(
				...
				fakeAsync(() => {
					itemList.items = [new FakeFocusable('едно'), new FakeFocusable('две'), new FakeFocusable('три')];
			...
			it(
				'should handle non-letter characters',
				fakeAsync(() => {
					itemList.items = [new FakeFocusable('[]'), new FakeFocusable('321'), new FakeFocusable('`!?')];
			...
			it(
				'should not focus disabled items',
				fakeAsync(() => {
			...
			it(
				'should start looking for matches after the active item',
				fakeAsync(() => {
					itemList.items = [
			...
			it(
				'should wrap back around if there were no matches after the active item',
				fakeAsync(() => {
					itemList.items = [
						new FakeFocusable('Bilbo'),
						new FakeFocusable('Frodo'),
			...
			it(
				'should wrap back around if the last item is active',
				fakeAsync(() => {
					keyManager.setActiveItem(2);
			...
			it(
				'should be able to select the first item',
				fakeAsync(() => {
					keyManager.setActiveItem(-1);
					keyManager.onKeydown(createKeyboardEvent('keydown', 79, undefined, 'o'));
					tick(debounceInterval);
			...
			it(
				'should not do anything if there is no match',
				fakeAsync(() => {
					keyManager.setActiveItem(1);
	...
	describe('FocusKeyManager', () => {
		...
		it('should allow setting the focused item without calling focus', () => {
			...
			expect(keyManager.activeItemIndex).toBe(
				1,
				`Expected activeItemIndex to update after calling updateActiveItemIndex().`
			);
