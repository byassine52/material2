...
describe('InteractivityChecker', () => {
	...
	describe('isDisabled', () => {
		it('should return true for disabled elements', () => {
			...
			elements.forEach(el => {
				expect(checker.isDisabled(el)).toBe(true, `Expected <${el.nodeName} disabled> to be disabled`);
			});
		});
		...
		it('should return false for elements without disabled', () => {
			...
			elements.forEach(el => {
				expect(checker.isDisabled(el)).toBe(false, `Expected <${el.nodeName}> not to be disabled`);
			});
		});
	});
	...
	describe('isVisible', () => {
		...
		it('should return false for the child of a `visibility: hidden` element', () => {
           ...
           <input>
			...
			expect(checker.isVisible(input)).toBe(
				false,
				'Expected element with `visibility: hidden` parent to not be visible'
			);
		});
	...
	describe('isFocusable', () => {
		...
		it('should return false for the child of a `visibility: hidden` element', () => {
           ...
           <input>
			...
			expect(checker.isFocusable(input)).toBe(
				false,
		...
		it('should return false for an element with a non-numeric tabindex', () => {
			...
			expect(checker.isFocusable(element)).toBe(
				false,
		...
		it('should return false for inert div and span', () => {
			...
			elements.forEach(el => {
				expect(checker.isFocusable(el)).toBe(false, `Expected <${el.nodeName}> not to be focusable`);
			});
		});
	});
	...
	describe('isTabbable', () => {
		it(
			'should respect the tabindex for video elements with controls',
		...
		it(
			'should always mark video elements with controls as tabbable (BLINK & FIREFOX)',
		...
		describe(
			'for Blink and Webkit browsers',
			runIf(platform.BLINK || platform.WEBKIT, () => {
		...
		describe(
			'for iOS browsers',
			runIf(platform.IOS && platform.WEBKIT, () => {
