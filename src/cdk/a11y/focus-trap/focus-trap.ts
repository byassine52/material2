...
import { Directive, ElementRef, Input, NgZone, OnDestroy, AfterContentInit, Injectable, Inject } from '@angular/core';
	...
	private _getRegionBoundary(bound: 'start' | 'end'): HTMLElement | null {
		...
		for (let i = 0; i < markers.length; i++) {
			if (markers[i].hasAttribute(`cdk-focus-${bound}`)) {
				console.warn(
					`Found use of deprecated attribute 'cdk-focus-${bound}',` + ` use 'cdkFocusRegion${bound}' instead.`,
					markers[i]
				);
			} else if (markers[i].hasAttribute(`cdk-focus-region-${bound}`)) {
				console.warn(
					`Found use of deprecated attribute 'cdk-focus-region-${bound}',` + ` use 'cdkFocusRegion${bound}' instead.`,
					markers[i]
				);
			}
		}
