...
describe('Overlay', () => {
	...
	it('should disable pointer events of the pane element if detached', () => {
		...
		expect(paneElement.style.pointerEvents).toBe(
			'auto',
			'Expected the overlay pane to enable pointerEvents when attached.'
		);
		...
		expect(paneElement.style.pointerEvents).toBe(
			'none',
			'Expected the overlay pane to disable pointerEvents when detached.'
		);
	});
	...
	it('should ensure that the most-recently-attached overlay is on top', () => {
		...
		expect(pizzaOverlayRef.overlayElement.nextSibling).toBeTruthy('Expected pizza to be on the bottom.');
		expect(cakeOverlayRef.overlayElement.nextSibling).toBeFalsy('Expected cake to be on top.');
		...
		expect(pizzaOverlayRef.overlayElement.nextSibling).toBeTruthy('Expected pizza to still be on the bottom.');
		expect(cakeOverlayRef.overlayElement.nextSibling).toBeFalsy('Expected cake to still be on top.');
	});
	...
	it('should emit the attachment event after everything is added to the DOM', () => {
		...
		overlayRef.attachments().subscribe(() => {
			expect(overlayContainerElement.querySelector('pizza')).toBeTruthy('Expected the overlay to have been attached.');

			expect(overlayContainerElement.querySelector('.cdk-overlay-backdrop')).toBeTruthy(
				'Expected the backdrop to have been attached.'
			);
		});
	...
	it('should emit the detachment event after the overlay is removed from the DOM', () => {
		...
		overlayRef.detachments().subscribe(() => {
			expect(overlayContainerElement.querySelector('pizza')).toBeFalsy('Expected the overlay to have been detached.');
		});
	...
	describe('positioning', () => {
		...
		it(
			'should apply the positioning strategy',
			fakeAsync(() => {
		...
		it(
			'should not apply the position if it detaches before the zone stabilizes',
			fakeAsync(() => {
				config.positionStrategy = new FakePositionStrategy();
	...
	describe('backdrop', () => {
		...
		it('should insert the backdrop before the overlay pane in the DOM order', () => {
			...
			expect(children.indexOf(backdrop)).toBeLessThan(
				children.indexOf(pane),
				'Expected backdrop to be before the pane in the DOM'
			);
		});
	});
	...
	describe('scroll strategy', () => {
		...
		it('should attach the overlay ref to the scroll strategy', () => {
			expect(fakeScrollStrategy.overlayRef).toBe(
				overlayRef,
