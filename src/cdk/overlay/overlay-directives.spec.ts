...
describe('Overlay directives', () => {
	...
	it(`should attach the overlay based on the open property`, () => {
		...
		expect(getPaneElement().style.pointerEvents).toBe(
			'auto',
	...
	it('should destroy the overlay when the directive is destroyed', () => {
		...
		expect(getPaneElement()).toBeFalsy('Expected the overlay pane element to be removed when disposed.');
	});
	...
	it('should use a connected position strategy with a default set of positions', () => {
		...
		let strategy = <ConnectedPositionStrategy>overlayDirective.overlayRef.getConfig().positionStrategy;
	...
	describe('inputs', () => {
		...
		it('should set the offsetY', () => {
			...
			expect(pane.style.top).toBe('95px', `Expected overlay translateY to equal the start Y + height + offsetY.`);
			...
			fixture.componentInstance.isOpen = true;
			fixture.detectChanges();
			expect(pane.style.top).toBe('105px', `Expected overlay directive to reflect new offsetY if it changes.`);
		});
	...
	describe('outputs', () => {
		...
		it('should emit attach and detach appropriately', () => {
			...
			expect(fixture.componentInstance.attachResult instanceof HTMLElement).toBe(
				true,
...
class ConnectedOverlayDirectiveTest {
	...
	attachHandler = jasmine.createSpy('attachHandler').and.callFake(() => {
		this.attachResult = this.connectedOverlayDirective.overlayRef.overlayElement.querySelector('p') as HTMLElement;
	});
