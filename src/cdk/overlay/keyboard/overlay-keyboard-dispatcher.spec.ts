...
describe('OverlayKeyboardDispatcher', () => {
	...
	it('should track overlays in order as they are attached and detached', () => {
		...
		expect(keyboardDispatcher._attachedOverlays.length).toBe(2, 'Expected both overlays to be tracked.');
		...
		expect(keyboardDispatcher._attachedOverlays[0]).toBe(overlayTwo, 'Expected two to now be first.');
		expect(keyboardDispatcher._attachedOverlays[1]).toBe(overlayOne, 'Expected one to now be last.');
	});
