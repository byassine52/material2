...
describe('CdkTable', () => {
	...
	it('should render cells even if row data is falsy', () => {
		...
		const booleanRowCdkTableElement = booleanRowCdkTableAppFixture.nativeElement.querySelector('cdk-table');
	...
	it('should be able to apply class-friendly css class names for the column cells', () => {
		...
		const crazyColumnNameTableElement = crazyColumnNameAppFixture.nativeElement.querySelector('cdk-table');
		crazyColumnNameAppFixture.detectChanges();

		// Column was named 'crazy-column-NAME-1!@#$%^-_&*()2'
		expect(getHeaderCells(crazyColumnNameTableElement)[0].classList).toContain(
	...
	it('should throw an error if two column definitions have the same name', () => {
		expect(() => TestBed.createComponent(DuplicateColumnDefNameCdkTableApp).detectChanges()).toThrowError(
	...
	it('should throw an error if the row definitions are missing', () => {
		expect(() => TestBed.createComponent(MissingRowDefsCdkTableApp).detectChanges()).toThrowError(
	...
	describe('using when predicate', () => {
		...
		it('should error if there is row data that does not have a matching row template', () => {
			...
			expect(() => whenFixture.detectChanges()).toThrowError(getTableMissingMatchingRowDefError().message);
		});
		...
		it('should error if there are multiple rows that do not have a when function', () => {
			...
			expect(() => whenFixture.detectChanges()).toThrowError(getTableMultipleDefaultRowDefsError().message);
		});
	});
	...
	describe('with trackBy', () => {
		...
		it('should add/remove/move rows with changed references without property-based trackBy', () => {
			...
			trackByComponent.dataSource.data = trackByComponent.dataSource.data.map(item => {
	...
	it('should match the right table content with dynamic data source', () => {
		...
		expectTableToMatchContent(tableElement, [['Column A'], [newData[0].a], [newData[1].a], [newData[2].a]]);
	});
...
class OuterTableApp {
	dataSource: FakeDataSource = new FakeDataSource();
	columnsToRender = ['content_column_a', 'content_column_b', 'injected_column_a', 'injected_column_b'];
...
function expectTableToMatchContent(tableElement: Element, expectedTableContent: any[]) {
	...
	function checkCellContent(cell: Element, expectedTextContent: string) {
		...
		if (actualTextContent !== expectedTextContent) {
			missedExpectations.push(`Expected cell contents to be ${expectedTextContent} but was ${actualTextContent}`);
		}
	}
	...
	getHeaderCells(tableElement).forEach((cell, index) => {
		const expected = expectedHeaderContent ? expectedHeaderContent[index] : null;
	...
	getRows(tableElement).forEach((row, rowIndex) => {
		getCells(row).forEach((cell, cellIndex) => {
			const expected = expectedTableContent.length ? expectedTableContent[rowIndex][cellIndex] : null;
			checkCellContent(cell, expected);
		});
	});
