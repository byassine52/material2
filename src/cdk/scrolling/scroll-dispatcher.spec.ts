...
describe('Scroll Dispatcher', () => {
	...
	describe('Basic usage', () => {
		...
		beforeEach(
			inject([ScrollDispatcher], (s: ScrollDispatcher) => {
				scroll = s;
		...
		it(
			'should notify through the directive and service that a scroll event occurred',
			fakeAsync(() => {
	...
	describe('Nested scrollables', () => {
		...
		it('should be able to identify the containing scrollables of an element', () => {
			...
			const scrollableElementIds = scrollContainers.map(scrollable => scrollable.getElementRef().nativeElement.id);
	...
	describe('lazy subscription', () => {
		...
		it('should remove global listeners on unsubscribe, despite any other live scrollables', () => {
			...
			expect(scroll.scrollContainers.size).toBe(4, 'Expected scrollable count to stay the same');
		});
