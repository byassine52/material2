...
export class TableDemo {
	...
	constructor(public _peopleDatabase: PeopleDatabase) {
		...
		this.matTableDataSource.filterPredicate = (data: UserData, filter: string) => data.name.indexOf(filter) != -1;
	}
	...
	isMasterToggleIndeterminate() {
		return (
			this.selection.hasValue() && (!this.isAllFilteredRowsSelected() || !this.matTableDataSource.filteredData.length)
		);
	}
	...
	connect() {
		...
		this.dataSource = new PersonDataSource(this._peopleDatabase, this.paginator, this.sort);
	...
	getOpacity(progress: number) {
		...
		return distanceFromMiddle / 50 + 0.3;
	}
