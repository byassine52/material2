...
export class OverlayDemo {
	...
	openKeyboardTracking() {
		...
		let overlayRef = this.overlay.create(config);
		const componentRef = overlayRef.attach(new ComponentPortal(KeyboardTrackingPanel, this.viewContainerRef));

		overlayRef
			.keydownEvents()
			.pipe(tap(e => (componentRef.instance.lastKeydown = e.key)), filter(e => e.key === 'Escape'))
